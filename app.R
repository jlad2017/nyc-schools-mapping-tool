                            ...
                            sidebarLayout(
                                sidebarPanel(
                                    ...
                                    tags$ul(
                                        ...
                                        tags$li('Air quality data is the 2018 annual average of PM2.5 in milligrams / cubic meters (mg/m3).'),
                                        tags$li('Pneumonia visits and admissions data are the total visits and admissions in 2020.')
                                    )
                                ),
                                ...
                                mainPanel(
                                    ...
                                    tags$br(),
                                    fluidRow(
                                        column(4, tableOutput('school_data')),
                                        column(4, tableOutput('area_data'))
                                    )
                                )
                            )
                        )
...
server <- function(input, output) {
    ...
    output$map <- renderLeaflet({
        leaflet() %>%
            ...
            addCircleMarkers(data        = schools_vis(),
                             ...
                             # options = highlightOptions(color        = "white",
                             #                            weight       = 2,
                             #                            bringToFront = F,
                             #                            opacity      = 0.7),
    ...
    # Render school data 
    school <- data.frame(Name  = c('School Name', school_dem_display_options), 
                         Value = double(length=length(school_dem_display_options)+1))
    output$school_data <- renderTable(school,
                                      caption  = 'School Demographic Data',
    ...
    observeEvent(input$map_marker_click, {
        ...
        names <- clean_name(c('School Name', school_dem_display_options))
        school$Value <- matrix(schools[names][click$id,])
    ...
    area_data <- reactive({
        if (input$area_type == 'air_q') {
            area_data_names <- c('UHF Neighborhood', 'Air Quality', 'Borough')
        } else {
    ...
    output$area_data <- renderTable(area_data(),
                                    caption  = 'Area Data',
                                    caption.placement = 'top',
                                    striped  = TRUE,
                                    hover    = TRUE,
    ...
    observeEvent(input$map_shape_click, {
        ...
        output$area_data <- renderTable(area,
                                        caption  = 'Area Data',
                                        caption.placement = 'top',
                                        striped  = TRUE,
                                        hover    = TRUE,
